...
const container = (props: Render.ContainerProps = { args: {} }): Render.Return => {
  ...
  if (tag === 'ul' || tag === 'ol') {
    ...
    classesArray.push('ls-none')
  }

  /* Max width */

  if (maxWidth !== '') {
    classesArray.push(`container${maxWidth !== 'default' ? `-${maxWidth}` : ''}`)
  }

  /* Flex */

  if (layout === 'column' && (justify !== '' || align !== '')) {
    classesArray.push('flex col')
  }

  if (layout === 'row') {
    classesArray.push('flex flex-wrap')
  }
  ...
  if (gap !== '') {
    if (layout === 'row') {
      classesArray.push(`gap-${gap}`)
    } else {
      classesArray.push(`m-bottom-${gap}-all`)
    }
  }

  if (gapLarge !== '' && gapLarge !== gap) {
    if (layout === 'row') {
      classesArray.push(`gap-${gapLarge}-l`)
    } else {
      classesArray.push(`m-bottom-${gapLarge}-all-l`)
    }
  }
  ...
  if (paddingTop !== '') {
    classesArray.push(`pt-${paddingTop}`)
  }

  if (paddingTopLarge !== '' && paddingTopLarge !== paddingTop) {
    classesArray.push(`pt-${paddingTopLarge}-m`)
  }

  if (paddingBottom !== '') {
    classesArray.push(`pb-${paddingBottom}`)
  }

  if (paddingBottomLarge !== '' && paddingBottomLarge !== paddingBottom) {
    classesArray.push(`pb-${paddingBottomLarge}-m`)
  }

  /* Rich text styles */

  if (richTextStyles) {
    classesArray.push('rich-text e-line-in')
